...
The entropy of a pure state is always 0. However, subsystems of a pure state are mixed, and therefore have non-zero entropy. This entropy does not represent uncertainty, since the state of the whole system is known. Rather, it represents the classical information between the two states~\cite{janzing09}. 
...
\end{align}
If the smaller of the two subsystems has $m$ particles, the sum in~\ref{eqn:mixed} can contain at most $2^m$ terms, the maximum number of states in the smaller subsystem. Maximum entropy is achieved when the coefficients $c_i$ are equal, so 
\begin{align}
...
It is particularly interesting to note that the number of ground states is particularly high for even $N$. Even with randomly generated Hamiltonians, the relative number of ground states above the bound stays relatively constant near 0.33 in this case. \_\_Why?\_\_
...
\begin{figure}
	...
	\caption{\textbf{Entanglement Entropy.} On the left, each curve shows the entropy for a given $N$ at various $M$. The straight line bounds the entropy as $m\log2$. The right plot shows the maximum entropy achieved for $0<N\le12$, along with the bounds $N\log\sqrt{3}$ and $\log\sqrt{2}$.}
	\label{fig:allentropy}
\end{figure}

Close to equipartition, the entropy is smaller. Figure~\ref{fig:allentropy} also shows the maximum entropy as a function of $N$.
