...
%\label{Kt/2}
%\ee
%for a time exponential in $K.$ At $ t\sim e^K$ the complexity reaches its maximum possible value $\CC_{max}$ and flattens out for a very long time. This is the period of \it complexity equilibrium \rm   \cite{Susskind:2015toa} during which the complexity fluctuates about the maximum,
%\be 
%\CC_\textrm{max} \sim e^K. \label{eq:Cmax}
%\ee
%On a much longer time scale of order $\exp[e^K]$ quantum recurrences quasiperiodically return the complexity to sub-exponential values. All of this is a conjecture which at the moment cannot be proved, but which can be related to other complexity conjectures  \cite{Aaronson}.
%
...
%
%In \cite{Brown:2016wib} a two-dimensional toy analog model for complexity was conjectured. (We recommend that the reader first read \cite{Brown:2016wib} before this paper.) The motivation for the toy model was Nielsen's geometric approach to complexity. Here we are going to consider the far more complex case  based directly on a version Nielsen's high-dimensional geometry \cite{Nielsen}\cite{Dowling}.  \\
%
%
%
%
...
%\eea
%(Here and throughout this paper we normalize $\Tr$ so that $\Tr \mathbb{1} = 1$.)
...
%The particle starts at the point $U= \mathbb{1}$, i.e., the identity matrix. The motion can be represented by ordinary classical mechanics. Begin with the Schrodinger equation for $U(t),$
%\be 
%i\dU = H U.
%\label{schrodinger}
%\ee
...
%%
%%
%At the origin  $U=  \mathbb{1}$ we may write
%\beq
...
%$|B\ra$ to make $ \alpha |A\ra + \beta |B\ra$ and then evolve the superposition forward for time $t.$ That is true, but the point is that  this way of preparing $ \alpha |A'\ra + \beta |B'\ra$ takes a very long time.  With some locality assumptions we can show that there is no faster way to do it.
%
%
%
...
%
%Certain basic facts about the evolution of complexity can be summarized by two properties of such sections (that are briefly reviewed in Appendix~\ref{sec: A vs D}):  the geometry should be uniformly negatively curved; and the magnitude of the curvature should be of order $1/K.$ Thus the typical sectional metric has the form
%\begin{equation} 
%dl^2 =  K ( dt^2 + \sinh^2{t} \ d\theta^2 ) = K \left(dt^2 + \sum_{n=1} C_n t^{2n} d\theta^2 \right) ,
%\label{toy}
%\end{equation}
%with $C_n$ some set of numerical coefficients. We will now compare this geometry  with the two-dimensional sectional curvatures of the complexity geometry defined by Eqs.~\ref{right-inv} \& \ref{Omega}, and see that the sectional geometries indeed have the same general form as the right side of Eq.~\ref{toy} (though not necessarily exactly the same $C_n$s). This may be regarded as a partial derivation of the toy model from the more complete complexity geometry.
%
%
...
%linear combinations of two $k$-local Hamiltonians. For definiteness we will choose $k=2$ for the remainder of this section. Without loss of generality we may choose one Hamiltonian to be $H$ and the other to be $H + \Delta d \theta$, where $\Delta$ is a $2$-local operator orthogonal to $H$ and $d\theta$ is an infinitesimal angle.
%
%The surface defined in this way will generally not have zero extrinsic curvature, and so geodesics connecting two points on the section will in general take short-cuts off the surface. For this reason, strictly speaking the sectional curvature is usually defined as the curvature at the origin, $t=0$, where this issue does not arise. We will mean something more general, namely the curvature of this section as a full function of $t$; the nonzero extrinsic curvature will not be an issue so long as we restrict ourselves to working at leading order in $d \theta$. 
%
%\begin{figure}[H]
%\begin{center}
...
% 
%(We have also confirmed \cite{UsInFuture} that for all \kl \ generalizations of Eq.~\ref{2L} the sectional curvature is generically $\sim k^2/K.$ Again, this is because the probability that two randomly chosen $k$-local terms share a qubit and therefore fail to commute is roughly $k^2/K$.) 
%
%
%Given $H$ and $\Delta$ the sectional curvature is a single number that describes the geometry of the section near the tangent plane where $t=0$. As we've seen it is only dependent on the first penalty factor $\CI_{3}$. A full derivation of the toy model would require an understanding of the dependence of the sectional curvature on the distance $t$ away from the origin. The Baker-Campbell-Hausdorff formula allows us to calculate the metric Eq.~\ref{d(t)} to arbitrary order in $t$.  We have not carried out a full calculation, but we have done enough to show the pattern is
%$$ 
%dl^2 = K \left( dt^2 +   \sum_{n=1}  \tilde{C}_{n} t^{2n}  \, d\theta^2 \right),
%\label{sixth}
%$$
%for some coefficients $\tilde{C}_n$. This reproduces both the form and the $K$-dependence of Eq.~\ref{toy}. The coefficients $\tilde{C}_n$ can be shown to depend on all penalty factors up to and including $\CI_{n+1}$; as we will show in \cite{UsInFuture}, the general pattern has the form
%\be 
%\tilde{C}_n = \sum_{m\leq n} (-1)^{m} C({m,n}) \CI_{m+1} ,
%\label{pattern}
%\ee
%with $C({m,n})>0.$ If follows that the curvature is uniformly of order $1/K$ and that for appropriate choices of the penalty factors $\CI_w$  can be made negative and constant. What we have not yet been able to show is that the penalty factors determined by this requirement grow exponentially as in Eq.~\ref{moderation} but we expect this to be the case.
%
%
%
%
...
% is the minimal geodesic distance separating it from the identity operator $\mathbb{1}$,  
% 
%\be
%\CC = \int_{\mathbb{1}}^U \sqrt{G_{MN}dX^M dX^N},
%\label{geo-length}
%\ee
%where the integral is taken along the shortest geodesic connecting ${\mathbb{1}}$ and $U.$
...
%\be 
%\CC = 
%\frac{1}{2}\int_{\mathbb{1}}^U G_{MN}\dot{X}^M\dot{X}^N d\tau,
%\label{quad-action}
%\ee
%with the constraint that the conserved energy $E_a$ is proportional to  the actual energy $E$ of the quantum system, 
%\be 
%E_a = K.
%\label{Ea=K/2}
%\ee
%
%
%We note that in Rindler units the total energy of a black hole is proportional to its entropy which may be identified with $K$.  The constraint  Eq.~\ref{Ea=K/2} says that the auxiliary energy is equal  to the true energy of the system $\CQ,$
%\be
%E_a = E.
%\ee
%
...
%In both the time-independent Hamiltonian model, and the repeated-$\Gamma$ model the Kolmogorov complexity is essentially a fixed overhead which does not grow linearly as the circuit evolves. Thus whether we include it or not, we may ignore it over long time scales. 
...
%
% Granting the correspondence between average complexity and auxiliary entropy, we can  give a rough analogy for the growth and evolution of computational  complexity. Initially a large number $\sim 2^K$ of particles are located near the origin of a large box of volume $e^{2^K}.$ The  velocities are Maxwell-Boltzmann distributed. The gas begins to expand and the positional entropy grows. Eventually  the gas fills the box and  comes to equilibrium. It stays in equilibrium for a very long time but on time scales $e^{2^K}$ recurrences happen. Figure~\ref{f1} is the result of translating this picture into the computational complexity of the system $\CQ$. 
%
%
%
%
...
%\ee
%Next apply the controlled $G$ operation $G_c = |1 \ra \la 1|  G + |0 \ra \la 0 | \mathbb{1}$, where $G = g_N g_{N-1}....g_1$; this circuit applies $G$ to $|{\rm{max}}\ra$ if the control qubit is $|1\ra$, and otherwise leaves it unchanged. This gives 
%\be 
...
%
%We'd like to thank Patrick Hayden and Scott Aaronson for patient tutoring and especially for introducing us to the idea of one-clean-qubit computation. We'd also like to thank Andy Albrecht,  Beni Yoshida, Dan Roberts, Daniel Harlow, Douglas Stanford, Henry Lin, Hrant Gharibyan, Nicole Yunger Halpern, Simeon Hellerman, Ying Zhao.  
%\sc
%\appendix
%
%
...
%\label{symmetric}
%\ee
%(The $\Tr$ notation denotes normalized trace, i.e., $\Tr {\mathbb{1}} = 1.$)
%
%\bn
...
%
%
%%\cite{Susskind:2014rva}
%\bibitem{Susskind:2014rva} 
%  L.~Susskind,
%  ``Computational Complexity and Black Hole Horizons,''
%  Fortsch.\ Phys.\  {\bf 64}, 24 (2016)
%%  doi:10.1002/prop.201500092
%  [arXiv:1403.5695 [hep-th], arXiv:1402.5674 [hep-th]].
%  %%CITATION = doi:10.1002/prop.201500092;%%
%  %61 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Stanford:2014jda}
%\bibitem{Stanford:2014jda} 
%  D.~Stanford and L.~Susskind,
%  ``Complexity and Shock Wave Geometries,''
%  Phys.\ Rev.\ D {\bf 90}, no. 12, 126007 (2014)
%%  doi:10.1103/PhysRevD.90.126007
%  [arXiv:1406.2678 [hep-th]].
%  %%CITATION = doi:10.1103/PhysRevD.90.126007;%%
%  %45 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Susskind:2014jwa}
%\bibitem{Susskind:2014jwa} 
%  L.~Susskind and Y.~Zhao,
%  ``Switchbacks and the Bridge to Nowhere,''
%  arXiv:1408.2823 [hep-th].
%  %%CITATION = ARXIV:1408.2823;%%
%  %19 citations counted in INSPIRE as of 04 Dec 2016
%
%%\cite{Brown:2016wib}      \cite{Susskind:2014jwa}
%\bibitem{Brown:2016wib} 
%  A.~R.~Brown, L.~Susskind and Y.~Zhao,
%  ``Quantum Complexity and Negative Curvature,''
%  arXiv:1608.02612 [hep-th].
%  %%CITATION = ARXIV:1608.02612;%%
%  %5 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%    \cite{Susskind:2015toa}
%\bibitem{Susskind:2015toa} 
%  L.~Susskind,
%  ``The Typical-State Paradox: Diagnosing Horizons with Complexity,''
%  Fortsch.\ Phys.\  {\bf 64}, 84 (2016)
%%  doi:10.1002/prop.201500091
%  [arXiv:1507.02287 [hep-th]].
%  %%CITATION = doi:10.1002/prop.201500091;%%
%  %8 citations counted in INSPIRE as of 04 Dec 2016
%
%
...
%  %538 citations counted in INSPIRE as of 29 Dec 2016
%
%
%%\cite{Aaronson}
%\bibitem{Aaronson} 
%S. Aaronson, L. Susskind, to appear.
%
%Reviewed in
% S.~Aaronson,
%  ``The Complexity of Quantum States and Transformations: From Quantum Money to Black Holes,''
%  arXiv:1607.05256 [quant-ph].
%  %%CITATION = ARXIV:1607.05256;%%
%  %1 citations counted in INSPIRE as of 06 Nov 2016
%
%See also  
%  Y.~Atia and D.~Aharonov,
%  ``Fast-forwarding of Hamiltonians and Exponentially Precise Measurements,''
%  arXiv:1610.09619 [quant-ph].
%  %%CITATION = ARXIV:1610.09619;%%
%
%%\cite{Nielsen}    \cite{Dowling}
%\bibitem{Nielsen} 
%Michael A. Nielsen, ``A geometric approach to quantum circuit lower bounds,''
%arXiv:quant-ph/0502070.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%\cite{Dowling}
%\bibitem{Dowling} 
%Mark R. Dowling, Michael A. Nielsen, ``The geometry of quantum computation''
...
%  %224 citations counted in INSPIRE as of 04 Dec 2016
%
%
...
%  %117 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Sachdev:1992fk}
...
%  %69 citations counted in INSPIRE as of 04 Dec 2016
%
%
%\bibitem{Kitaev} 
%A.	Kitaev. A simple model of quantum holography - 2015. KITP strings seminar and Entanglement program (Feb. 12, April 7, and May 27, 2015). http://online.kitp.ucsb.edu/online/entangled15/
%
%%\cite{Polchinski}
%\bibitem{Polchinski} 
...
%  %%CITATION = doi:10.1007/JHEP04(2016)001;%%
%  %21 citations counted in INSPIRE as of 09 Sep 2016
%
%%\cite{Maldacena:2016hyu}
...
%  %219 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Lashkari:2011yi}
...
%  %%CITATION = doi:10.1007/JHEP04(2013)022;%%
%  %61 citations counted in INSPIRE as of 04 Dec 2016
%
%\bibitem{UsInFuture}
%A.~R.~Brown and L.~Susskind, 
%``Complexity Geometry'', in preparation.
%
%
%%\cite{Brown:2015bva}
...
%  %40 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Brown:2015lvg}
%\bibitem{Brown:2015lvg} 
%  A.~R.~Brown, D.~A.~Roberts, L.~Susskind, B.~Swingle and Y.~Zhao,
%  ``Complexity, action, and black holes,''
%  Phys.\ Rev.\ D {\bf 93}, no. 8, 086006 (2016)
%%  doi:10.1103/PhysRevD.93.086006
%  [arXiv:1512.04993 [hep-th]].
%  %%CITATION = doi:10.1103/PhysRevD.93.086006;%%
%  %26 citations counted in INSPIRE as of 04 Dec 2016
%
%
%
%
...
%Advances in Mathematics  21, 293-329 (1976).
%
%%\cite{Aharonov:1987gg}
%\bibitem{Aharonov:1987gg} 
...
%  %%CITATION = doi:10.1103/PhysRevLett.58.1593;%%
%  %297 citations counted in INSPIRE as of 04 Dec 2016
%
%
%%\cite{Lloyd}
%\bibitem{Lloyd} 
%
%
% S. Lloyd, ``Computational Capacity of the Universe'', Physical Review Letters {\bf 88} 237901 (June, 2002), quant-ph/0110141; 
%S. Lloyd, ``A theory of quantum gravity based on quantum computation'', arXiv:quant-ph/0501135;  S. Lloyd, ``Quantum limits to the measurement of spacetime geometry'', arXiv:quant-ph/0505064.
%
%%\cite{Zurek:1989zz}
%\bibitem{Zurek:1989zz} 
%  W.H.~Zurek,
%  ``Algorithmic randomness and physical entropy,''
%  Phys.\ Rev.\ A {\bf 40}, 4731 (1989).
%%  doi:10.1103/PhysRevA.40.4731
%  %%CITATION = doi:10.1103/PhysRevA.40.4731;%%
%  %21 citations counted in INSPIRE as of 04 Dec 2016
%
%%\cite{Grunwald}
%\bibitem{Grunwald}
%P.D. Grunwald, P.M.B. Vitanyi, ``Shannon Information and Kolmogorov complexity'', IEEE Trans. Information Theory, arXiv:cs/0410002.
%
%%\cite{Loschmidt}
%\bibitem{Loschmidt}
%A.~Goussev, R.~Jalabert, H.~Pastawski, D.~Wisniacki, ``Loschmidt Echo'', arXiv:1206.6348.
%
%
%%\cite{Beni}
%\bibitem{Beni} 
...
%  %84 citations counted in INSPIRE as of 04 Dec 2016
%
%\bibitem{Spekkens}
%G.~Gour, M.~P.~M\"uller, V.~Narasimhachar, R.~W.~Spekkens, N.~Yunger~Halpern,
%``The resource theory of informational nonequilibrium in thermodynamics",
%arXiv:1309.6586;
%
%F.~G.~S.~L.~Brandao, M.~Horodecki, J.~Oppenheim, J.~M.~Renes, R.~W.~Spekkens, 
%``The Resource Theory of Quantum States Out of Thermal Equilibrium'', Phys. Rev. Lett. {\bf 111}, 250404 (2013);
%
%V.~Veitch, S.~A.~H.~Mousavian, D.~Gottesman, J.~Emerson, 
%``The Resource Theory of Stabilizer Computation'', New J. Phys. 16 (2014) 013009.
%
%
%
%
%%\cite{Knill:1998wi}
%\bibitem{Knill:1998wi} 
%  E.~Knill and R.~Laflamme,
%  ``On the power of one bit of quantum information,''
%  Phys.\ Rev.\ Lett.\  {\bf 81}, 5672 (1998)
%%  doi:10.1103/PhysRevLett.81.5672
%  [quant-ph/9802037].
%  %%CITATION = doi:10.1103/PhysRevLett.81.5672;%%
%  %14 citations counted in INSPIRE as of 06 Dec 2016
%  
%  See also 
%  
%   P. Shor and S. P.~Jordan, 
...
%  %%CITATION = ARXIV:1203.5813;%%
%  %2 citations counted in INSPIRE as of 06 Dec 201
%
%K.~Fujii, H.~Kobayashi, T.~Morimae, H.~Nishimura, S.~Tamate, S.~Tani, 
%``Impossibility of Classically Simulating One-Clean-Qubit Computation'', arXiv:1409.6777 [quant-ph].
%
%
%
%\bibitem{Bennett1993}
%C.~H.~Bennett, P.~Gacs, M.~Li, P.M.B.~Vitanyi, and W.H.~Zurek, ``Thermodynamics of computation and information distance,''
%Proceedings of STOC '93, 21-30 (1993).
%%DOI=http://dx.doi.org/10.1145/167088.167098
%
%
%%\cite{Landauer}
%\bibitem{Landauer}
%R. Landauer, ``Irreversibility and heat generation in the computing process’'', IBM J. Res. Develop., 183–191 (1961).
%
%
%%\cite{Chemissany:2016qqq}
%\bibitem{Chemissany:2016qqq} 
%  W.~Chemissany and T.~J.~Osborne,
%  ``Holographic fluctuations and the principle of minimal complexity,''
%  arXiv:1605.07768 [hep-th].
%  %%CITATION = ARXIV:1605.07768;%%
%  %2 citations counted in INSPIRE as of 04 Dec 2016
%
%
%\end{thebibliography}
%
